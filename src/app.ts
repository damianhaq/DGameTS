...
game.onMouseWheel = (e) => {
  ...
  const cellsOnLeft =
    (game.camera.x + game.canvas.width / 2) / GAME.map.cellLength;
  // ile kratek jest na górze ekranu
  const cellsOnTop =
    (game.camera.y + game.canvas.height / 2) / GAME.map.cellLength;
  ...
  if (e.deltaY < 0) {
    ...
    // zoom out
    if (GAME.map.cellLength <= 10) return; // dont zoom out more than 10
    GAME.map.cellLength -= zoomFactor;
    game.camera.x -= zoomFactor * cellsOnLeft;
    game.camera.y -= zoomFactor * cellsOnTop;
  }
...
game.draw = (deltaTime) => {
  if (GAME.map.showMapGrid) map.drawGrid();
