export class Building {
    constructor(posCells, id, color, draw, gameVariables) {
    ...
    drawBuilding() {
        this.posCells.forEach((build, index, arr) => {
            ...
            this.drawRect(build.cellX, build.cellY, !neig.isTop, !neig.isRight, !neig.isBottom, !neig.isLeft);
        });
    }
    drawRect(cellX, cellY, isTop, isRight, isBottom, isLeft) {
        // points in corners
        const p1 = {
        ...
        const p2 = {
            x: cellX * this.gameVariables.map.cellLength +
        ...
        const p3 = {
            x: cellX * this.gameVariables.map.cellLength +
                this.gameVariables.map.cellLength -
                this.lineWidth / 2,
            y: cellY * this.gameVariables.map.cellLength +
                this.gameVariables.map.cellLength -
                this.lineWidth / 2,
        };
        const p4 = {
        ...
        if (isTop) {
            // top line
            this.draw.line(p1.x - this.lineWidth / 2, p1.y, p2.x + this.lineWidth / 2, p2.y, true, this.color, this.lineWidth);
        }
        if (isRight) {
            // right line
            this.draw.line(p2.x, p2.y - this.lineWidth / 2, p3.x, p3.y + this.lineWidth / 2, true, this.color, this.lineWidth);
        }
        if (isBottom) {
            // bottom line
            this.draw.line(p3.x + this.lineWidth / 2, p3.y, p4.x - this.lineWidth / 2, p4.y, true, this.color, this.lineWidth);
        }
        if (isLeft) {
            // left line
            this.draw.line(p4.x, p4.y + this.lineWidth / 2, p1.x, p1.y - this.lineWidth / 2, true, this.color, this.lineWidth);
        }
    }
    //   private outlineCell(
    //     cellX: number,
